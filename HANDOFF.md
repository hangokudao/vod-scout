...
- 자동 검증 PASS: `cmd.exe /c npm.cmd test` 39개, `cmd.exe /c npm.cmd run build`, `cargo.exe test --manifest-path src-tauri/Cargo.toml` 93 passed·1 ignored, `cargo.exe test --manifest-path src-tauri/fixture-worker/Cargo.toml` 5 passed, `git diff --check`.
- 검증 HOLD: 실제 선택 후보 미디어를 이용한 Whisper 재실행·GPU/CPU 대체 증거와 실제 Windows UI 흐름은 입력·환경이 없어 실행하지 않았다.
