...
1. 다음 패치 계획에서 NSIS updater 뒤 HKCU `DisplayVersion`이 갱신되지 않는 조건을 격리 재현한다.
2. 장시간 측정 도구가 병합 중 열린 출력 파일 길이를 안전하게 표본 수집하도록 보완한 뒤 내려받기 순간 최대 임시 용량을 다시 측정한다.
3. 그 외 v0.4.0 기능, 검색 기능, 새 기능, 리팩터링은 별도 승인과 계획 없이 시작하지 않는다.
