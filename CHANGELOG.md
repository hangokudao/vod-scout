...
아래 항목은 계획만 확정됐으며 아직 구현·배포되지 않았다. 실제 변경이 확인되면 해당 버전의 `Added`, `Changed`, `Fixed`, `Security`, `Known issues`로 옮긴다.

### v0.3.4 계획

- 설정 버튼 가시성, 다크 모드 입력 카드 대비, 취소 완료 지연을 수정한다.
- updater 뒤 Windows 제거 프로그램 `DisplayVersion`과 내려받기 병합 중 임시 용량 측정을 바로잡는다.
- 상세 범위와 완료 조건은 `docs/V0.3.4-PLAN.md`를 따른다.
