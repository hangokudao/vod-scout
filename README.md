...
긴 버튜버 VOD에서 쇼츠 후보를 먼저 좁혀 주는 Windows용 오프라인 편집 보조 앱입니다.
...
- CPU 전사이므로 영상 길이와 PC 성능에 따라 상당한 시간이 걸릴 수 있음
...
- 영상 처리: 다운로드가 끝난 뒤 전사·점수 계산은 PC 안에서 실행
- 작업 데이터: `%LOCALAPPDATA%/com.vodscout.app/jobs/{job-id}`
- 임시 WAV: 청크 처리 후 삭제
- 보존 파일: 작업 스냅샷, JSONL 활동 로그, 체크포인트, 전사 JSON, 도구 진단 로그
