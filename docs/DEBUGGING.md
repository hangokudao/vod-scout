...
- 필수 게이트의 첫 실패는 ZJ release-app job `6251521b-6749-4415-9bf1-7eac826bae0f`의 `scripts/e2e-local-cdp.mjs:279` player-ready 검사다. download `100%`, `REVIEW_READY`, 후보 `20`, `18/18` units, GPU `12/12` completed까지는 확인했지만 이 게이트 뒤 screenshot·preview·후보 재인식·전체 UI는 검증하지 못해 `HOLD`/`BLOCKED`로 판정했다. JSON/log basename은 `C:\Users\myhan\AppData\Local\Temp\vod-scout-v050-public-gate-20260820\evidence-zjmp-retry\e2e-failure-2026-08-19T19-24-56-054Z-17056`다.
- 두 번째 JKY release-app job `c8f80e03-33d7-4c3b-af5f-6f498be31f72`도 download `100%`, `REVIEW_READY`, 후보 `20`, `22/22` units, GPU `16/16` completed 뒤 같은 player-ready 게이트에서 실패했다. JSON/log basename은 `C:\Users\myhan\AppData\Local\Temp\vod-scout-v050-public-gate-20260820\evidence-jky\e2e-failure-2026-08-19T19-39-18-521Z-24076`다. 이는 제품의 HTTP 403 원인을 확정하지 않으며 추가 E2E 재실행 근거가 아니다.
...
- provenance 이후 최종 회귀는 npm 49, Rust `128 passed·1 ignored`, fixture `6`, npm security `6`, standalone Node `6/3/3`, build와 `git diff --check` `PASS`다. 전체 로그·exit code는 `C:\Users\myhan\AppData\Local\Temp\vod-scout-final-post-provenance-vxCIf4`에 있다.
- package build 증거는 EXE/NSIS PE `0.5.0`, 각각 `4bcac97a30edb54be83b81711524ad4335cce5bdc315a44ff77d53121f52ec74`/`7c9e013794886fc220d82e1503d97277e5d79afca2b97f6ec7d367eab3041d3e`이며 Authenticode는 `NotSigned`다. 서명·설치·공개 Release는 `HOLD`; docs-only 변경 뒤 package rebuild는 하지 않는다.
