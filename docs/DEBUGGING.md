...
## 2026-08-05 · v0.3.3 · 다크 모드 입력 카드 대비 부족

- 사용자 제보와 재현 화면: 다크 모드의 새 작업 화면에서 선택하지 않은 `데모`와 `YouTube` 입력 카드가 밝은 회색으로 표시되고 글자와 아이콘도 밝아, 내용을 읽기 어렵고 사용할 수 없는 항목처럼 보인다.
- 확인한 원인: 다크 모드는 본문 글자를 밝은 색으로 바꾸지만 `.source-tabs button` 배경은 테마 변수 대신 `rgba(255, 255, 255, 0.58)`로 고정돼 있다. 밝은 배경과 밝은 글자가 겹쳐 대비가 부족해진다.
- 앞으로 할 일: 입력 카드 배경을 밝은 화면·어두운 화면에 맞는 테마 변수로 교체하고, 기본·마우스 올림·선택·비활성화·키보드 포커스 상태를 두 화면 설정에서 각각 확인한다. 입력 카드 제목·설명·아이콘과 비활성화된 시작 버튼도 실제 화면에서 구분 가능한지 함께 검증한다.
- 수정: 아직 하지 않음.
- 상태: `HOLD`
