...
- 조치: 추측성 앱 레지스트리 수정 훅을 넣지 않았다. 제품 버전을 `0.3.4`로 정렬한 뒤, 제어된 설치·updater 재현에서 ARP·PE가 모두 `0.3.4`인지 확인하는 게이트를 둔다. 재현이 실패할 때만 최소 수정(템플릿/Install 경로 우선, 최후 단일 HKCU 값)을 검토한다.
- 상태: `HOLD` — 원인 미확정, 재현 게이트 대기
